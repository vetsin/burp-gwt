...
class BurpExtender(IBurpExtender, IScannerInsertionPointProvider):
    ...
    def	registerExtenderCallbacks(self, callbacks):
        ...
        callbacks.setExtensionName("GWT Scanner")
        callbacks.registerScannerInsertionPointProvider(self)
        return
        
    def getInsertionPoints(self, baseRequestResponse):
        """ implement IScannerInsertionPointProvider """    
        # if the request type is of GWT
        request = self._helpers.analyzeRequest(baseRequestResponse)
        for header in request.getHeaders():
            if "x-gwt-rpc" in header:
                gwt = GWTParser()
                baseRequest = baseRequestResponse.getRequest()
                offset = request.getBodyOffset()
                gwt.deserialize(self._helpers.bytesToString(baseRequest)[offset:])
                injections = gwt.get_injections()
                print("injections:")
                print(injections)
                ret = []
                for start,end in injections:
                	#print("injecting on: %s" % (self._helpers.bytesToString(baseRequest)[offset+start:offset+end]))
                	ret.append(InsertionPoint(self._helpers, baseRequestResponse.getRequest(),start,end))
                return ret
        return None
        
class InsertionPoint(IScannerInsertionPoint):

    def __init__(self, helpers, baseRequest, start, end):
        self._helpers = helpers
        self._baseRequest = baseRequest
        self._offset = self._helpers.analyzeRequest(baseRequest).getBodyOffset()
        self._start = start
        self._end = end
        return

    def getInsertionPointName(self):
        return "GWT String Point"

    def getBaseValue(self):
        return self._helpers.bytesToString(self._baseRequest)[self._offset + self._start:self._offset + self._end]

    def buildRequest(self, payload):
        # build the raw data using the specified payload
        req = self._helpers.bytesToString(self._baseRequest)
        newRequest = req[:self._offset + self._start]
        newRequest += self._helpers.bytesToString(payload).replace('\\', '\\\\')
        newRequest += req[self._offset + self._end:]

        return newRequest
        
    def getPayloadOffsets(self, payload):
        return [self._offset + self._start, self._offset + self._start + len(payload)]
...
class GWTParser(object):
    ...
    def get_injections(self):
    	''' Just get the fuzzstr, return all the indicies for %s '''
        original = self.rpc_string.split('|')
        original.pop() # make same length as rpc_list_fuzzable
        offsets = []
        i = 0
        lastindex = 0
        for o,f in zip(original, self.rpc_list_fuzzable):
            if f == "%s":
                start = self.rpc_string.find(o, lastindex)
                end = start + len(o)
                offsets.append((start,end))
            i = i+1 # keep track of what element we're on
            lastindex = self.rpc_string.find('|', lastindex+1)
        return offsets
